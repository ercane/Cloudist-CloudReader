...
public class ImageLoaderTask {
    ...
    private final Map<String, SoftReference<Bitmap>> Cache = new HashMap<String,
            SoftReference<Bitmap>>();
    private final List<QueueItem> Queue = new ArrayList<QueueItem>();
    private final Handler handler = new Handler();  // Assumes that this is started from the main
    // (UI) thread
    ...
    public Bitmap loadImage(final String uri, final ImageLoadedListener listener) throws
        ...
        //item.key= Base64.encodeToString(uri.getBytes(),Base64.DEFAULT);
        ...
        public void run() {
            synchronized (this) {
                while (Queue.size() > 0) {
                    ...
                    if (Cache.containsKey(item.url.toString()) && Cache.get(item.url.toString())
                            != null) {
                        ...
                        handler.post(new Runnable() {
                            public void run() {
                                if (item.listener != null) {
                                    ...
                                    // cache item could get
                                    //     garbage collected between when we post the runnable
                                    // and it's executed.
