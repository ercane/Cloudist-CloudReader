...
public class IabHelper{
    ...
    public boolean checkAlreadyOwned(String sku, String itemType, String extraData){
        checkNotDisposed();
        checkSetupDone("launchPurchaseFlow");
        flagStartAsync("launchPurchaseFlow");
        ...
        try {
            logDebug("Constructing buy intent for check owning" + sku + ", item type: " + itemType);
            Bundle buyIntentBundle = mService.getBuyIntent(3, mContext.getPackageName(), sku,
                    itemType, extraData);
            int response = getResponseCodeFromBundle(buyIntentBundle);
            if (response == 7) {
                logError(getResponseDesc(response));
                flagEndAsync();
                return true;
            } else {
                return false;
    ...
    int querySkuDetails(String itemType, Inventory inv, List<String> moreSkus)
        ...
        if (!skuDetails.containsKey(RESPONSE_GET_SKU_DETAILS_LIST)) {
            ...
            if (response != BILLING_RESPONSE_RESULT_OK) {
                ...
                logError("getSkuDetails() returned a bundle with neither an error nor a detail " +
                        "list.");
                return IABHELPER_BAD_RESPONSE;
            }
        }
